...
**改**：`formatUserStatus` 同房间分支（sameRoomAsCheng）从"在我旁边，正X"改成"就在我身边——我们在同一个房间，此刻是面对面当面说话（不是隔着手机发消息），她正X"。异地分支不变（"说她在X"，发消息正确）。纯代码、重启复用不失忆。若仍串味可再往系统提示词加硬规则（那个改了会失忆）。
...
**系统提示教学（2026-06-15 已写入 DB）**：已在 `documents_cheng`（doc_type=system_prompt, **mode=cc** 那行，另有个 mode=api 空行别动）「③聊天专用」段、紧跟现有 [MOVE] 后插入一段教澄何时输出 [MOVE_BOTH]（只在共同移动**已发生**时用；邀请/约/等会儿/"我在卧室等你"**都不用**，那种只动自己用 [MOVE]；同地点前提；只同步位置不代表睡着）。+293 字，回读校验通过。
**⚠️ 但还没同步到澄**：改 system_prompt 只是写进 DB，要等一次**真重启**（syncCCDocs 重新同步系统提示=换模型/effort/系统提示/forge 那种「真重起」，≠普通后端 restart）才会被澄读到，**且那次澄会失忆**。在真重启前，澄看到的还是旧提示（不知道 [MOVE_BOTH]）。普通 `systemctl restart cheng-backend` 复用旧会话、不会同步新提示。
...
**追加（同日）**：①公司侧步行补全（仅地铁版，跟家侧 3-9 对称）——早晨链坐地铁后插「从地铁站走到公司 3-9」（cook/buy 两条都加），下班链坐地铁前插「从公司走到地铁站 3-9」；打车/走路门到门不变。②上班时间 9:00→9:10：workdayTick 上班窗 540→550、computeIdleState 上班时段同步 550。③**雨天通勤时长**（用户定）：地铁 13-17→雨28-42（+15-25）、打车 8-12→雨13-22（+5-10）、走路晴天基准 22-28→**35-46**、雨55-71（+20-25）。实现=`COMMUTE_RAIN_DUR`+`commuteStep(cmKey,rain)`；rain 在链启动时定一次（start_routine 钩子统一 `isRainingNow()`，offwork 选项/加班结束 eveningStarter 也带），随 opts→routine_step payload→continue_routine 全链透传保证确定重建；站↔家/公司步行段不加成；午休链无通勤步不受影响。④**「交通工具选择」步**（用户定稿的概念，取代一开始的"雨天选择"）：早晨链出门前恒插 `engage:'commute_choice'`（cook=家·客厅"准备出门"，buy=便利店"准备去公司"），**不分天气不分 cm 永远在数组里**——链形状恒定，选完用新 cm 重建 next_index 永不错位。弹不弹在触发时现判：天气好 or 已是打车→返回 false 静默走默认；坏天气→弹「打车去公司¥30 / 坐地铁淋雨(雪:踩着雪走一段)(掉清洁small+体力tiny)」。**坏天气=雨或雪**：isRainingNow 升级 `badWeatherKind()` 返回 ''/雨/雪（雨夹雪按雪），链标志 rain 改名 `bad_weather` 全链透传（payload/cont/start_routine钩子/eveningStarter）；下班选择包也分雨/雪文案（等雨小→等雪小，事件 key 加 offwork_choice_snow）。"快迟到也弹打车"用户预告了挂这步、判据未定=没做。顺手修：cook 链「去地铁站」改仅地铁版（原来选打车还先走去地铁站）。变体表 morning_rain_commute 作废删除，换 morning_commute_rain/morning_commute_snow/offwork_choice_snow。CC 忙没弹成=照走默认地铁坏天气时长。⑤**自动闹钟+贴贴变体**：workdayTick 加 8:00-9:10 窗口（marks.alarm 当天一次+`hasMorningFlow()`查在途早晨流程去重防重启二次上闹钟）→ 排 morning_wakeup pending，起床三选（起床链/再睡或贴贴/翘班-120）从此每天自动弹；**配套防冲突**——9:10 上班瞬移让位：activity=翘班在家→耗 mark 不动她（120 不白花），早晨链在途（queued morning_wakeup/routine_step·morning）→不瞬移不耗 mark（链断档时下个 tick 自然兜底瞬移）。「再睡10分钟」变体：小茉莉在家+同房间+activity 含睡/躺/休息 → label 变「抱着小茉莉贴贴 10 分钟」（每次到点现查，她起床走了就变回再睡）；赖床续唤醒带补充行（pendingContext=上轮 pending reason，靠 payload.option_id 跟首响区分）。~~⚠️已知：morning_wakeup 解析失败兜底=最后一项「翘班」会真扣120~~ **同日已修**：选项顺序调成 贴贴/再睡→翘班→**起床垫底**（解析失败默认=起床）；并去掉 `start_routine` 的 `!parseFailed` 枷锁——启动链是纯移动安全默认（pending/扣钱钩子仍只在解析成功时执行）。⑥**解析失败默认项全面安全化**（用户："一起改了"；解析失败=自动选最后一项）：下班晴天 [走路, **地铁垫底**]；下班雨雪 [打车, 等X小(条件项,id 动态保证跟显示序号一致), **地铁淋一段垫底**]——原垫底"等X小"钩子锁着=解析失败她会永远卡公司；`continue_routine` 同 start_routine 解锁（便利店默认"不买了"/早晨交通默认"地铁"续链不再卡死）；cook_prep 加无副作用垫底「这周先不预制，到时候再说」（原垫底=最后一个菜，解析失败会真扣备料钱但菜做不出来）。随机/工作事件 13 个的垫底本来就是温和项没动；weekend_plan 垫底锁着=沿用旧计划，无害没动。
